...
def _convert_message_to_gemini_content(
    ...
    if not parts:
        parts.append({"text": "Generate responsive frontend code for this UI design."})
...
class GeminiProviderSession(ProviderSession):
    def __init__(
        ...
        if len(prompt_messages) > 1 and prompt_messages[0].get("role") == "system":
            self._system_prompt = str(prompt_messages[0].get("content", ""))
            user_msgs = prompt_messages[1:]
        else:
            self._system_prompt = ""
            user_msgs = prompt_messages

        self._contents: List[types.Content] = [
            _convert_message_to_gemini_content(msg) for msg in user_msgs
        ]
        if not self._contents:
            self._contents = [
                types.Content(
                    role="user",
                    parts=[types.Part.from_text(text="Generate responsive frontend code for this UI design.")],
                )
            ]
    ...
    async def append_tool_results(
        ...
        api_model_name = _get_gemini_api_model_name(self._model)
        # Gemini 2.5 Flash does not support multimodal function responses.
        # Only pass multimodal parts for models that explicitly support them (gemini-3.x+).
        supports_multimodal_responses = "gemini-3" in api_model_name

        tool_result_parts: List[types.Part] = []
        for executed in executed_tool_calls:
            multimodal_parts: List[types.FunctionResponsePart] = []
            for part in (executed.result.multimodal_parts or []) if supports_multimodal_responses else []:
            ...
            res_val = executed.result.result
            response_dict = res_val if isinstance(res_val, dict) else {"result": res_val}

            func_resp_kwargs: Dict[str, Any] = {
                "id": executed.tool_call.id,
                "name": executed.tool_call.name,
                "response": response_dict,
            }
            if multimodal_parts:
                func_resp_kwargs["parts"] = multimodal_parts
