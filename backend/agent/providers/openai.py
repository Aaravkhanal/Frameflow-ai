...
class OpenAIProviderSession(ProviderSession):
    def __init__(
        ...
        override_api_name: str | None = None,
        override_reasoning_effort: str | None = None,
    ):
        self._client = client
        self._model = model
        self._tools = tools
        self._override_api_name = override_api_name
        self._override_reasoning_effort = override_reasoning_effort
        self._total_usage = TokenUsage()
        
        api_model_name = self._override_api_name or get_openai_api_name(model)
    ...
    async def stream_turn(self, on_event: EventSink) -> ProviderTurn:
        model_name = self._override_api_name or get_openai_api_name(self._model)
        params: Dict[str, Any] = {
            "model": model_name,
        ...
        reasoning_effort = self._override_reasoning_effort
        if reasoning_effort is None and not self._override_api_name:
            # Only fetch default reasoning effort if we aren't overriding the API name
