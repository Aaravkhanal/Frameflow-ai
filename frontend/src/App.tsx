...
function App() {
  ...
  function doGenerateCode(
    ...
    generateCode(wsRef, updatedParams, {
      ...
      onAgentStart: (data) => appendOrchestrationEvent({ agentStart: data }),
      onAgentComplete: (data) => appendOrchestrationEvent({ agentComplete: data }),
      onAgentCritique: (data) => appendOrchestrationEvent({ agentCritique: data }),
      onDebateRound: (data) => appendOrchestrationEvent({ debateRound: data }),
      onPipelineStage: (data) => appendOrchestrationEvent({ pipelineStage: data }),
    });
  }
  ...
  return (
    <div
      className={`bg-gradient-to-br from-slate-950 via-slate-900 to-zinc-950 text-zinc-100 ${
        appState === AppState.CODING || appState === AppState.CODE_READY
    ...
    >
      ...
      {isCodingOrReady && !isSettingsOpen && (
        <div className="border-b border-white/10 bg-slate-900/80 backdrop-blur-xl px-4 py-2 lg:hidden">
      ...
      {showContentPanel && !isSettingsOpen && (
        <div
          className={`border-b border-white/10 bg-slate-950/80 backdrop-blur-2xl text-white lg:fixed lg:inset-y-0 lg:left-16 lg:z-40 lg:flex lg:w-[calc(28rem-4rem)] lg:flex-col lg:border-b-0 lg:border-r ${
