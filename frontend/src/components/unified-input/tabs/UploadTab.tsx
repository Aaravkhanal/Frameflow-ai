...
function UploadTab({ doCreate, stack, setStack, designSystem }: Props) {
  ...
  const dropzoneClassName = useMemo(() => {
    const base =
      "flex flex-1 w-full min-h-[320px] flex-col items-center justify-center p-5 border-2 border-dashed rounded-xl bg-slate-900/60 backdrop-blur-xl text-zinc-300 outline-none transition-all cursor-pointer shadow-xl";
    if (isFocused) {
      return `${base} border-cyan-500 bg-cyan-950/30`;
    }
    if (isDragAccept) {
      return `${base} border-emerald-500 bg-emerald-950/30`;
    }
    if (isDragReject) {
      return `${base} border-red-500 bg-red-950/30`;
    }
    return `${base} border-white/10 hover:border-cyan-500/40`;
